...
osThreadId_t osThreadNew(osThreadFunc_t func, void *argument, const osThreadAttr_t *attr)
{
    ...
    else
    {
        ...
        thread_cb->prio = (rt_uint8_t)attr->priority;
    }
    ...
    rtt_prio = (rt_uint8_t)((osPriorityISR - thread_cb->prio) * RT_THREAD_PRIORITY_MAX / osPriorityISR);
...
osStatus_t osThreadSetPriority(osThreadId_t thread_id, osPriority_t priority)
{
    ...
    thread_cb->prio = (rt_uint8_t)priority;
    rt_priority = (rt_uint8_t)((osPriorityISR - thread_cb->prio) * RT_THREAD_PRIORITY_MAX / osPriorityISR);
...
osStatus_t osDelayUntil(uint32_t ticks)
{
    ...
    {
        rt_thread_delay((rt_tick_t)(ticks - cur_ticks));
    }
    else
    {
        rt_thread_delay((rt_tick_t)(((rt_uint32_t) - 1) - cur_ticks + ticks));
    }
...
osMessageQueueId_t osMessageQueueNew(uint32_t msg_count, uint32_t msg_size, const osMessageQueueAttr_t *attr)
{
    ...
    if ((RT_NULL == attr) || (RT_NULL == attr->mq_mem))
    {
        ...
        mq_size = (msg_align_size + sizeof(void *)) * msg_count;
...
/// \param[in]     timeout       \ref CMSIS_RTOS_TimeOutValue or 0 in case of no time-out.
/// \return status code that indicates the execution status of the function.
#if defined(RT_VERSION_CHECK) && (RTTHREAD_VERSION >= 50001)
